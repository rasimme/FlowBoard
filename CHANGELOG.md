...
- **Work can be moved from OpenClaw, and FlowBoard still decides (T-498).** Three new Gateway actions change a task's status or work state, approve a task in review, and reject one back to in-progress with a required reason. FlowBoard authorizes every one of them server-side: the review gate still refuses review → done outside the approve action, a task another agent actively holds still cannot be moved from outside, and FlowBoard's own refusal reaches the caller word for word. Approvals and rejections are recorded against the signed-in OpenClaw profile, never against a name the browser supplied.
