...
- Set the task's **work state** separately from its lifecycle status: *Active* (the default — no chip shown), *Waiting*, *Blocked*, or *Paused*. Waiting/blocked/paused states can include a reason, who/what is next, a responsible person, and a check-again time. The server owns the `setAt` timestamp; client-supplied values are ignored. The detail panel shows the claiming agent and the work state together as one combined chip — click either half to open claim actions or the state picker. A blocked subtask also shows a small lock-icon pill on its card.
- If monitoring needs attention, the detail panel shows one **attention banner** above Activity/Checkpoints, including its detected time. It is updated in place rather than added as a comment, and dismissing it ("x") clears the underlying signal server-side — it reappears on its own if the condition persists. The banner appears only when the backend provides explicit non-destructive action descriptors; during backend rollout it may be absent until that contract is available.
- The **active agents** bar above the board shows who currently holds a claim, the task they're on, its progress, and a lease-health indicator; open a row's popover to see every claim that agent holds.
