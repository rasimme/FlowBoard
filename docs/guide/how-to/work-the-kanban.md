...
- Set the task's **work state** separately from its lifecycle status in the detail panel: *Working*, *Waiting*, *Blocked*, or *Paused*. Waiting/blocked/paused states can include a reason, who/what is next, a responsible person, and a check-again time. The server owns the `setAt` timestamp; client-supplied values are ignored. The legacy `blocked` flag shown on cards is a read-only projection of the canonical *Blocked* state.
