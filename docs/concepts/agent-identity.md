...
- **No auth boundary on *which* id you assert.** Anyone with access to the dashboard port can pose as any agent-id. This is intentional for the personal-tool deployment model. Don't expose the dashboard to a network you don't trust.
- **Lease ownership *is* enforced, even though identity is not.** Trust-on-write means the server does not verify *which* agent-id a local caller claims to be — it does **not** mean a caller can act on another agent's work. Lifecycle ownership is enforced server-side: a caller asserting agent `X` cannot `complete`, `checkpoint`, or `release` a task currently leased to agent `Y` (rejected with `NOT_OWNER`), nor `claim` a task another agent holds under a live, unexpired lease (`ALREADY_CLAIMED`). Stealing a claim is allowed only once that lease has expired or the task has been released. So "pose as any agent-id" lets a local caller *attribute new work* to a name — it does not let it override another agent's active lease or rewrite its task. See `dashboard/hzl-service.js` (`claimTask`, `completeTask`, `addCheckpoint`, `releaseTask`) and the regression guard `dashboard/test-lease-ownership.js`.
