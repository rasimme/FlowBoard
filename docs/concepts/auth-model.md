...
**Local development should work without ceremony.** Setting up Telegram bots, JWT secrets, and allowed user lists just to poke the API on `localhost` is hostile to dogfooding. The default fail-closed behavior allows `127.0.0.1` requests through unauthenticated when auth is unconfigured. The same holds in production (`NODE_ENV=production`) as long as the server is bound to loopback: it starts with a warning that the dashboard is unauthenticated and loopback-only. What production refuses is a non-loopback bind with auth off (T-509; ADR-0028 as amended).
...
- **Production without auth is loopback-only.** Booting in `NODE_ENV=production` without a fully configured auth stack works only on a loopback bind, and logs a warning saying so. A non-loopback bind in production exits the process, even with `FLOWBOARD_ALLOW_LAN=true` — that opt-in only applies outside production. Remote access in production needs the complete ordered bot-identity mapping, JWT secret, and allowed-user list.
- **Cloudflare Tunnel is the canonical remote-access path.** The middleware uses `cf-ray` as a fail-closed auth-routing marker, while `FLOWBOARD_TRUSTED_PROXY_IPS` controls whether forwarded client-IP headers may influence rate limiting. Using a different reverse proxy (ngrok, Tailscale Funnel, vanilla nginx) means the operator should use `AUTH_ALWAYS=true` and leave Cloudflare forwarding trust unset unless that proxy peer is explicitly verified and configured.
