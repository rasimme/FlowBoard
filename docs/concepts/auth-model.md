...
2. **Telegram init-data.** Requests from a Mini App send the `x-telegram-init-data` header — Telegram's signed user payload. The middleware HMAC-verifies it against every configured bot token (S-01 timing-safe, with every position evaluated), checks the `auth_date` is within 5 minutes, parses the `user` payload, rejects if the user id is not in `ALLOWED_USER_IDS`, and binds the matched token position to its ordered `FLOWBOARD_TELEGRAM_AGENT_IDS` entry. `POST /api/auth` is a strict exchange: supplied init-data always wins over a cookie; invalid, unsupported, future-dated, or expired data rejects and clears the existing cookie. Non-auth API calls may use an already valid cookie after otherwise valid init-data becomes older than five minutes, so a long-lived WebApp does not fail when Telegram's one-time payload naturally ages.
...
**Failed auth is typed and secret-safe.** Every failed credential check returns a stable code (for example `TELEGRAM_BOT_NOT_SUPPORTED`, `TELEGRAM_INIT_DATA_EXPIRED`, or `TELEGRAM_USER_NOT_ALLOWED`) and writes that code with `cf-connecting-ip` (or `req.ip`) and a timestamp. Neither response nor log contains bot-token values. There is no automatic exponential backoff or IP block; Express-rate-limit middleware is configured globally on `/api/` for general DOS protection.
