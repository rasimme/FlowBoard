...
**Work state (four values).** `workState` is orthogonal to the lifecycle column: `working`, `waiting`, `blocked`, or `paused`. The detail panel edits it without moving the task between columns. `workStateDetails` carries optional `reason`, `waitingFor`, `responsible`, and `checkAgainAt` context; `setAt` is server-owned. The legacy top-level `blocked` boolean was retired server-side (T-452-7): it is neither accepted nor returned by the API. `working`/Active is the invisible default state (no chip, no label); `waiting`/`blocked`/`paused` render as an icon + label.

In the DetailPanel, the agent (who holds the claim) and the work state are shown as one combined "combo chip" (T-452-1/2/3) rather than two separate controls — clicking either half opens the matching popover (claim actions, or the work-state picker). On subtask cards, a dedicated `BlockedChip` still renders a neutral, lock-icon pill when the canonical state is `blocked`; it reads `workState`, never the retired boolean.
...
A task may also carry one transient structured stuck indicator. In the DetailPanel it renders as a single merged **attention banner** (T-452-8, `AttentionWarning.jsx`) in the header — facts-only, one dismiss ("x"), no action buttons of its own (Waiting/Blocked/Paused already live in the combo chip, and a redundant "Retry" would just duplicate the scheduler's own re-evaluation). It is updated in place by monitoring, not a comment or historical resolution entry. `detectedAt` identifies the start of the current incident and remains stable across re-evaluations. The dismiss action is wired to the backend's explicit `POST` action descriptors for the exact project/task routes `/api/projects/{project}/tasks/{id}/stuck-indicator/{clear|retry}`; those non-destructive actions return a complete canonical task. The UI never synthesizes a work-state PUT or dismisses the indicator only in local state. During a rolling backend integration, missing or generic descriptors deliberately leave the banner hidden.
...
- **Multi-agent visibility is structural.** The UI's per-task agent chip and the active-agents bar both read from `tasks_current.agent` and `flowboard_agents` directly. There is no separate "presence" or "online" signal — *who is holding a lease* is the presence signal. An idle agent has no claims, so they're not visible on the board until they pick something up. The active-agents bar (project-local, above the board) lists one row per claiming agent with the claimed task's title, its subtask progress, and a lease-health dot (`current`/`stale`/`expired`, shared logic with the per-card indicators); a popover per row lists every claim that agent holds, each with its own relative-time-since-checkpoint.
...
- `dashboard/src/components/DetailPanel.jsx` — per-task drawer with lifecycle/status and work-state pickers, claim/release controls, comment thread, checkpoints, and the attention banner.
- `dashboard/src/components/AgentChip.jsx`, `PriorityPill.jsx` — visual primitives for the per-task metadata. `ClaimStateLine.jsx` (agent half) and `WorkStateChip.jsx`/`WorkStatePopover.jsx` (state half) make up the DetailPanel combo chip; `BlockedChip.jsx` is the card-level equivalent for subtasks; `AttentionWarning.jsx` renders the merged banner; `ActiveAgentsBar.jsx` renders the per-project claim list above the board.
