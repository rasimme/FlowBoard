...
A task may also carry one transient structured stuck indicator. It lives in the Activity/Checkpoint area, is updated in place by monitoring, and is not a comment or historical resolution entry. `detectedAt` identifies the start of the current incident and remains stable across re-evaluations. Retry/Clear controls are rendered only when the backend supplies explicit same-origin `POST` action descriptors; those non-destructive actions return a complete canonical task. The UI never synthesizes a work-state PUT or dismisses the indicator only in local state. During a rolling backend integration, missing descriptors deliberately leave the controls hidden.
