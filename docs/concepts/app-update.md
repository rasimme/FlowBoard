...
- `POST /api/update/run` shells out to a **fixed command** — `node scripts/setup.mjs --update` — not an arbitrary string, so the endpoint can't be coerced into running something else. `setup.mjs --update` reinstalls dependencies, rebuilds the UI, preserves the existing standard launchd/systemd environment, and restarts the service. Persisted values take precedence over the updater's inherited shell environment; named operator overrides require the CLI-only `--override-env` flag. Existing auth/tunnel/custom variables and `JWT_SECRET` are preserved; secret rotation requires the separate operator-only `--rotate-secret` flag.
