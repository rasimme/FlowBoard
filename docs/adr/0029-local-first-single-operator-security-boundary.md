...
- Code: the v5.0.x hardening guards — lease ownership on the claim-lifecycle paths (`dashboard/hzl-service.js`, `dashboard/server.js`), typed confirmation + append-only audit log (`dashboard/audit-log.js` and the destructive endpoints), the boot bind guard (`dashboard/server.js`, `dashboard/host-utils.js`).
...
The decisive fact: **OpenClaw — the host FlowBoard plugs into — is single-user / personal by design** (OpenClaw's own README/VISION describe a "personal AI assistant"; "multi-user" appears only as a lock-it-down caveat). Its "multi" is multi-*channel* and multi-*agent* under one human operator, not multi-*human*. It has no user accounts, no tenants, and no role model. FlowBoard runs as a plugin for that one operator. There is therefore no second human principal *inside the product's model* to authorize against.
...
- **We DO enforce, server-side, the invariants that prevent *accidental* cross-agent damage and keep actions answerable** — the legitimate residue of the audit under this model:
  - **Lease ownership on the claim-lifecycle paths.** claim / checkpoint / complete / release reject a non-owner (`NOT_OWNER`, HTTP 403); the generic `PUT /tasks/:id` status path now does too (T-422-1) — gated on a *live* lease (an active `claimedAt`/`leaseUntil`), not the historical attribution chip, so reopening a finished task by another actor is not blocked. An actor-less caller is the trusted operator/UI (its Status-Picker auto-release is preserved); an explicit `adminOverride` is the deliberate operator force-flag. **Intentionally outside this gate:** review transitions (`/approve`, `/reject`), routing, move, and reparent are operator/reviewer actions by a principal distinct from the worker, so they are not owner-gated by design — `agentId` there is attribution, consistent with the rest of this model.
