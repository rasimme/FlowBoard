...
> **Amendment (T-509, 2026-09-25):** "production fails closed" now means production refuses a
> *non-loopback* bind with auth disabled. A loopback-bound production dashboard with auth disabled
> starts, with a boot warning that it is unauthenticated and loopback-only. Under
> [ADR-0029](0029-local-first-single-operator-security-boundary.md) loopback is the operator in every
> `NODE_ENV`, and requiring Telegram to use production mode repeated the T-447 mistake recorded in
> [ADR-0035](0035-task-form-not-authorization.md): it made one optional channel a precondition for
> every install. The network exposure this clause guarded against is still refused.

## Status
Accepted (retroactive 2026-06-16, T-402-4) — amended by T-509 (2026-09-25): production fails closed on non-loopback binds only
...
Loopback is trusted by default so unconfigured local use just works. Production (`NODE_ENV=production`) **fails closed**: the server refuses to boot with auth disabled on a non-loopback bind (amended by T-509; a loopback bind starts with a warning). There is no user DB and no role model — once a request is admitted, every endpoint trusts the caller equally; `agentId` is attribution, not authorization (see [ADR-0003](0003-dashboard-has-no-agent-identity.md)).

## Consequences
- Local dogfooding needs zero auth setup; exposure requires explicit configuration, or the server won't start in production. A loopback-only production dashboard needs no auth setup either (T-509).
